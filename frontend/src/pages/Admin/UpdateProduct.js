...
const UpdateProduct = () => {
    ...
    const toggleVisibility = async () => {
        try {
            const { data } = await axios.put(
                `http://localhost:8080/api/product/update-product-visibility/${id}`,
                { isVisible: !isVisible }
            );
            if (data?.success) {
    ...
    const getSingleProduct = async () => {
        try {
            ...
            setIsVisible(data.product.isVisible);  // Set visibility status here
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const handleDelete = async () => {
        try {
            let answer = window.confirm("Are You Sure want to delete this App ? ");
            if (!answer) return;
                        ...
                        <div className="m-1 w-75">
                            ...
                            <div className="mb-3">
                                <input
                                    type="number"
                                    value={price}
                                    placeholder="write a Rating" min={1} max={5}
                                    className="form-control"
                                    onChange={(e) => setPrice(e.target.value)}
                                />
                            </div>
                            <div className="mb-3">
                                <button className={`btn ${isVisible ? "btn-danger" : "btn-success"}`} onClick={toggleVisibility}>
